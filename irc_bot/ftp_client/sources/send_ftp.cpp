...
int fileRecv(int sock, std::string fileName, std::queue<std::string> &msg_q);
...
int fileRecv(int sock, std::string fileName, std::queue<std::string> &msg_q)
{
    ...
    if (a < lenght - 1)
    {
        ...
        while (rem)
        {
            ...
            byteread = recv(sock, buffer, (rem < 1024) ? rem : 1024, 0);
            if (byteread < 0)
            {
...
int main(int ac, char *av[])
{
    ...
    else if (ac == 6 && std::string(av[4]) == "0")
    {
        startSender(av[1], atoi(av[2]), av[3], av[5]);
        return 0;
    }
